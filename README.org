...
*Q:* Why bother?  *A:* Small shell utiities will be more easily used to display
simple analyses if the user needs to install fewer dependencies.
*Q:* What dependencies are needed?  *A:* for maximum portability and
installation convenience, [[https://www.opengroup.org/posix-systems][POSIX-compliant]] methods will be used unless
the mechanism doesn't exist.
*Q:* Won't this be slow? *A:* probably. This is about convenience for the
user as a means of avoiding installing R when they need to run a small
utility.
